...
AddEventHandler("QBCore:Client:OnPlayerLoaded", function()
    ...
    QBCore.Functions.TriggerCallback('ksuie_robbery:server:GetConfig', function(config)
        Config = config
    end)
    onDuty = true
    ResetBankDoors()
                                        ...
                                        QBCore.Functions.Progressbar("hack_gate", "Connecting electronic kit..", math.random(5000, 10000), false, true, {
                                            ...
                                            if not copsCalled then
                                                ...
                                                if Config.SmallBanks[closestBank]["alarm"] then
                                                    TriggerServerEvent("ksuie_robbery:server:callCops", "small", closestBank, streetLabel, pos)
                                                    copsCalled = true
                                                end
                                            end
                                        end, function() -- Cancel
...
RegisterNetEvent('ksuie_robbery:client:setBankState')
AddEventHandler('ksuie_robbery:client:setBankState', function(bankId, state)
...
RegisterNetEvent('ksuie_robbery:client:enableAllBankSecurity')
AddEventHandler('ksuie_robbery:client:enableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = true
    end
end)

RegisterNetEvent('ksuie_robbery:client:disableAllBankSecurity')
AddEventHandler('ksuie_robbery:client:disableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = false
    end
end)

RegisterNetEvent('ksuie_robbery:client:BankSecurity')
AddEventHandler('ksuie_robbery:client:BankSecurity', function(key, status)
...
function openLocker(bankId, lockerId)
    ...
    TriggerServerEvent('ksuie_robbery:server:setLockerState', bankId, lockerId, 'isBusy', true)
    if bankId == "paleto" then
        QBCore.Functions.TriggerCallback('qb-ifruitstore:server:GetItem', function(hasItem)
            if hasItem then
                ...
                IsDrilling = true
                QBCore.Functions.Progressbar("open_locker_drill", "Cracking the safe..", math.random(40000, 60000), false, true, {
                    disableMovement = true,
                    disableCarMovement = true,
                    disableMouse = false,
                    disableCombat = true,
                    ...
                    TriggerServerEvent('ksuie_robbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
                    TriggerServerEvent('ksuie_robbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                    TriggerServerEvent('ksuie_robbery:server:recieveItem', 'paleto')
    ...
    elseif bankId == "pacific" then
        QBCore.Functions.TriggerCallback('qb-ifruitstore:server:GetItem', function(hasItem)
            if hasItem then
                ...
                QBCore.Functions.Progressbar("open_locker_drill", "Cracking the safe..", math.random(40000, 60000), false, true, {
                    ...
                    TriggerServerEvent('ksuie_robbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
                    TriggerServerEvent('ksuie_robbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                    TriggerServerEvent('ksuie_robbery:server:recieveItem', 'pacific')
                    QBCore.Functions.Notify("Succes!", "success")
                    IsDrilling = false
                end, function() -- Cancel
                    StopAnimTask(GetPlayerPed(-1), "anim@heists@fleeca_bank@drilling", "drill_straight_idle", 1.0)
        ...
        IsDrilling = true
        QBCore.Functions.Progressbar("open_locker", "Cracking the safe..", math.random(24000, 48000), false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            TriggerServerEvent('ksuie_robbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
            TriggerServerEvent('ksuie_robbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            TriggerServerEvent('ksuie_robbery:server:recieveItem', 'small')
...
RegisterNetEvent('ksuie_robbery:client:setLockerState')
AddEventHandler('ksuie_robbery:client:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "paleto" then
...
RegisterNetEvent('ksuie_robbery:client:ResetFleecaLockers')
AddEventHandler('ksuie_robbery:client:ResetFleecaLockers', function(BankId)
...
RegisterNetEvent('ksuie_robbery:client:robberyCall')
AddEventHandler('ksuie_robbery:client:robberyCall', function(type, key, streetLabel, coords)
...
function OnHackDone(success, timeremaining)
    if success then
        ...
        TriggerServerEvent('ksuie_robbery:server:setBankState', closestBank, true)
    else
		TriggerEvent('mhacking:hide')
	end
end
