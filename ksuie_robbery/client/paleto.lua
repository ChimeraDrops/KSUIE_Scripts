            ...
            if GetDistanceBetweenCoords(pos, Config.BigBanks["paleto"]["thermite"][1]["x"], Config.BigBanks["paleto"]["thermite"][1]["y"], Config.BigBanks["paleto"]["thermite"][1]["z"], true) < 10.0 then
                ...
                if not Config.BigBanks["pacific"]["thermite"][1]["isOpened"] then
                    ...
                    if dist < 1 then
                        ...
                        if GetDistanceBetweenCoords(pos, Config.BigBanks["paleto"]["thermite"][2]["x"], Config.BigBanks["paleto"]["thermite"][2]["y"], Config.BigBanks["paleto"]["thermite"][2]["z"], true) < 10.0 then
                            inRange = true
                            if not Config.BigBanks["pacific"]["thermite"][2]["isOpened"] then
                                local dist = GetDistanceBetweenCoords(pos, Config.BigBanks["paleto"]["thermite"][2]["x"], Config.BigBanks["paleto"]["thermite"][2]["y"], Config.BigBanks["paleto"]["thermite"][2]["z"], true)
                                if dist < 1 then
                                    currentThermiteGate = Config.BigBanks["paleto"]["thermite"][2]["doorId"]
                                    if not requiredItemsShowed2 then
                                        requiredItemsShowed2 = true
                                        TriggerEvent('inventory:client:requiredItems', requiredItems2, true)
                                    end
...
RegisterNetEvent('ksuie_robbery:UseBankcardA')
AddEventHandler('ksuie_robbery:UseBankcardA', function()
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    local requiredItems = {
        [1] = {name = QBCore.Shared.Items["security_card_01"]["name"], image = QBCore.Shared.Items["security_card_01"]["image"]},
                        ...
                        QBCore.Functions.Progressbar("security_pass", "Validitating card..", math.random(5000, 10000), false, true, {
                            ...
                            if not copsCalled then
                                ...
                                if Config.BigBanks["paleto"]["alarm"] then
                                    TriggerServerEvent("ksuie_robbery:server:callCops", "paleto", 0, streetLabel, pos)
                                    copsCalled = true
                                end
                            end
                        end, function() -- Cancel
