...
function CountCops()
    ...
    for i=1, #xPlayers, 1 do
        ...
        if  xPlayer.Functions.SetJob == 'police' then
...
if not moneytruck then
    if isRobbing and item.count > 0 and amount > 0 then
        ...
        if CopsConnected >= Config.Copsneeded then
            ...
            --TriggerClientEvent('QBCore:Notify','~r~Not Enough Police', source, r)
         end
    else
        isRobbing = false
...
QBCore.Functions.CreateUseableItem('security_card_02', function(source)
    TriggerClientEvent('qb-truckrobbery:rob', source)
end)
...
AddEventHandler('RS7x:Payout', function()
    ...
    while Robbing == true do
        ...
        if timer >= Config.Timer then
            Robbing = false
            break
            TriggerClientEvent('qb-truckrobbery:unfreeze', source)
        end
    end
end)
