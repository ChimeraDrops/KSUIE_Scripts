...
function callPolice(coords)
    ...
    --TriggerServerEvent('police:server:cornerselling', exports['cd_dispatch']:GetPlayerInfo(), vector3(0,0,0))
...
AddEventHandler('qb-drugs:client:refreshAvailableDrugs', function(items)
    QBCore.Functions.TriggerCallback('qb-drugs:server:cornerselling:getAvailableDrugs', function(result)
        if result ~= nil then
            ...
            QBCore.Functions.Notify('You sold all drugs..', 'error')
            cornerselling = false
        end
    end)
end)
...
function SellToPed(ped)
    ...
    local getRobbed = math.random(1, 30)
    ...
    if hasTarget then
        while pedDist < 1.5 do
            ...
            if getRobbed == 18 or getRobbed == 9 then
                ...
                QBCore.Functions.Notify('You have been robbed and lost '..bagAmount..' bag(\'s) '..availableDrugs[drugType].label, 'error')
                stealingPed = ped
                stealData = {
